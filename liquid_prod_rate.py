...
class FigureDrawer(object):
    ...
    def start_draw(self, t0, t1):
        self.th = threading.Thread(target=self.md_fill_rate, args = (t0, t1))
        self.th.start()
    ...
    def md_fill_rate(self, t0, t1):
        ...
        surplus = np.diff(mins)/np.diff(epoch_min_times)*3600*12.6
        ...
        # save data to file
        times_center = [t.timestamp() for t in times_center]
        dataframes = [pd.DataFrame({"epoch_time":df_orig.index.values, "lvl204 (%)":df_orig.lvl204.values}),
                      pd.DataFrame({"epoch_time":times_center, "rate (L/h)":rates, "drate (L/h)":drates}),
                      pd.DataFrame({"epoch_time":xpts, "surplus (L/h)":surplus}),
                      pd.DataFrame({"epoch_time":times_center, "flow (SLM)":return_flows, "dflow (SLM)":dreturn_flows})]
        filenames = ["levels.csv",
                     "rates.csv",
                     "surplus.csv",
                     "flows.csv"]
        
        for df, filen in zip(dataframes, filenames):
            with open(filen, 'w') as fid:
                fid.write('# Liquifier Production Data\n')
                fid.write(f'# Start time: {pd.to_datetime(t0, unit="s")}\n')
                fid.write(f'# End time:   {pd.to_datetime(t0, unit="s")}\n')
                fid.write(f'# File written {datetime.datetime.now()}\n')
            df.to_csv(filen, mode='a', index=False)
